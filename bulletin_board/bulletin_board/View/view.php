...
<body>
  <fieldset style="border: 1 solid; width:370px;">
    ...
    <form action="#" method="GET">
      ...
      <input type="text" id="title" name="<?= nameOfPostData::BOARD_TITLE ?>" value="<?= $postData[1]->title ?>" disabled>
      <label for="userId">작성자</label>
      <input type="text" id="userId" name="<?= nameOfPostData::USER_ID ?>" value="<?= $postData[1]->user_name ?>" disabled>
      <label for="reg_data">작성일</label>
      <input type="text" id="reg_data" name="<?= nameOfPostData::BOARD_REG_DATE ?>" value="<?= date_format(date_create($postData[1]->reg_date), "Y년 m월 d일") ?>" disabled>
      <label for="text">본문</label>
      <textarea id="text" name="<?= nameOfPostData::BOARD_CONTENTS ?>" style="resize: none" placeholder="<?= $postData[1]->contents ?>" disabled></textarea>
      ...
      <?php if (checkUserBalid($_SESSION['board_id'])) : ?>
        <!-- 글수정 -->
        <input type="submit" id="submit" value="글수정" formaction="<?= boardAddrInfo::FILENAME_MODIFY ?>">
        <!-- 글삭제 -->
        <input type="submit" id="submit" value="글삭제" formaction="<?= boardAddrInfo::FILENAME_DELETE_PROCESS ?>">
      <?php endif; ?>
    </form>
    <br>
    <hr>
